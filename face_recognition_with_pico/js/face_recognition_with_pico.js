document.addEventListener('DOMContentLoaded', function () {
    ...
    */
     ...
     */
    var all_sx = new Array(); //新增四個array存放每次的x, j, w, h
    var all_sy = new Array();
    var all_sw = new Array();
    var all_sh = new Array();
    ...
    function render_cliped_canvas(dets, i) { // dets == pico.cluster_detections()
        ...
        // transform: scaleX(-1); sx和sw之間的關係相反
        let sx = Math.round(x - w/2 + adjustment_of_x); //矩形左上角x座標
        let sy = Math.round(y - w/2 + adjustment_of_y) * 0.92; //矩形左上角y座標
        let sw = w + adjustment_of_w; //矩形的寬度
        let sh = w + adjustment_of_h + 0.08 * sy; //矩形的高度
        ...
        if(i >= 1){
            for(let j=i-1; j>=0; j--){
                if(sx >= all_sx[j] && sx <= all_sx[j] + all_sw[j]
                && sy >= all_sy[j] && sy <= all_sy[j] + all_sh[j]
                && (sw <= all_sw[j] + all_sx[j] - sx || sw <= sx - all_sx[j])
                && (sh <= all_sh[j] + all_sy[j] - sy || sh <= sy - all_sy[j])) return;
            }
        }
