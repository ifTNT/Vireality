<template>
    <div class="registration">
        <div class="profile" v-if="pageState===0">
            ...
            <p>{{errMessage}}</p>
            <img class="nextPage" src="/static/media/rightarrow.png" @click="nextPage"/>
        </div>
        <div class="consentForm" v-if="pageState===1">
            ...
            <div>
                {{ errNotAgree }}
                <img class="nextPage" src="/static/media/rightarrow.png" @click="nextFaceFeature"/>
            </div>
        ...
        <div class="camera" v-if="pageState===2">
            ...
            <div>
                <img class="nextPage" src="/static/media/rightarrow.png" @click="finish"/>
            </div>
        </div>
    </div>
...
export default {
  ...
  watch:{
      "checkPassword" : function() {
          this.correctPassword = (this.checkPassword===this.password)? true:false
      },
      "password" : function() {
          this.correctPassword = (this.checkPassword===this.password)? true:false
      }
   ...
   methods:{
       ...
       nextPage(){
           ...
           this.errMessage=""
           this.errAccount=""
           if(this.correctPassword && this.userId.length >=1 && this.userId.length <=20 && this.password.length >=4 && this.password.length <=20){
                axios.post(server.apiUrl("/user/findAccount"),
                {
                    uid: this.userId,
                })
                .then(
       ...
       finish(){
            console.log("finish")
            axios.post(server.apiUrl("/user/createAccount"),
            {
                uid: this.userId,
