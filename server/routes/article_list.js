...
router.get("/geolocation", async function(req, res, next) {
  ...
  try{
    ...
    if (err) { throw err}
    if(user.length === 0 || user[0].location.longitude === undefined || user[0].location.latitude === undefined) { throw "Not have this user or no lon or lat" }
    ...
    articles.forEach(target =>{
      ...
      if(s<10) /* [ALERT,TODO]:先丟一張照片出去 避免錯誤 */
        dis.push({id:target.article_id , thumbnail: target.thumbnail[0] ,lon:target.location.longitude,
          lat:target.location.latitude, author:target.author})
    })
    console.log("DIS:\n",dis)
    res.json({
      ok: "true",
